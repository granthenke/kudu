  ...
  while (rem > 0) {
    ...
    for (size_t i = completed_iov; i < iov_size; i++) {
      if (bytes_rem >= iov[i].iov_len) {
        ...
        // Partially read this result. Adjust the iov_len and iov_base to request
        // only the missing data and add to the next request.
...
class PosixWritableFile : public WritableFile {
 public:
  ...
  virtual Status AppendV(const vector<Slice> &data) OVERRIDE {
    ThreadRestrictions::AssertIOAllowed();
    Status s = DoWriteV(fd_, filename_, filesize_, data);
