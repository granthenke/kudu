...
public class KuduSerDe extends AbstractSerDe {
  ...
  public void initialize(Configuration sysConf, Properties tblProps)
    ...
    try (KuduClient client = KuduHiveUtils.getKuduClient(conf)) {
      // TODO: Option to use Hive schema instead of lookup?
      schema = client.openTable(tableName).getSchema();
    } catch (KuduException ex) {
      throw new SerDeException("Error opening Kudu table: " + tableName, ex);
    }
  ...
  private static PrimitiveTypeInfo toHiveType(Type kuduType, ColumnTypeAttributes attributes)
      throws SerDeException {
    switch (kuduType) {
      case BOOL: return TypeInfoFactory.booleanTypeInfo;
      case INT8: return TypeInfoFactory.byteTypeInfo;
      case INT16: return TypeInfoFactory.shortTypeInfo;
      case INT32: return TypeInfoFactory.intTypeInfo;
      case INT64: return TypeInfoFactory.longTypeInfo;
      ...
      case FLOAT: return TypeInfoFactory.floatTypeInfo;
      case DOUBLE: return TypeInfoFactory.doubleTypeInfo;
      case STRING: return TypeInfoFactory.stringTypeInfo;
      case BINARY: return TypeInfoFactory.binaryTypeInfo;
      default:
  ...
  public KuduWritable serialize(Object obj, ObjectInspector objectInspector) throws SerDeException {
    ...
    for(int i  = 0; i < schema.getColumnCount(); i++) {
      ...
      if (value == null) {
        ...
        ObjectInspector inspector = fields.get(i).getFieldObjectInspector();
        switch (type) {
          case BOOL:
          ...
          case UNIXTIME_MICROS:
            Timestamp timestampVal = ((TimestampObjectInspector) inspector)
  ...
  public Object deserialize(Writable writable) throws SerDeException {
    ...
    for(int i  = 0; i < schema.getColumnCount(); i++) {
      ...
      switch (typeInfo.getPrimitiveCategory()) {
        case BOOLEAN: output.add(new BooleanWritable((boolean) javaObj)); break;
        case BYTE: output.add(new ByteWritable((byte) javaObj)); break;
        case SHORT: output.add(new ShortWritable((short) javaObj)); break;
        case INT: output.add(new IntWritable((int) javaObj)); break;
        case LONG: output.add(new LongWritable((long) javaObj)); break;
        case TIMESTAMP: output.add(new TimestampWritable((Timestamp) javaObj)); break;
        case DECIMAL:
          HiveDecimal hiveDecimal = HiveDecimal.create((BigDecimal) javaObj);
          output.add(new HiveDecimalWritable(hiveDecimal)); break;
        case FLOAT: output.add(new FloatWritable((float) javaObj)); break;
        case DOUBLE: output.add(new DoubleWritable((double) javaObj)); break;
        case STRING: output.add(new Text((String) javaObj)); break;
        case BINARY: output.add(new BytesWritable((byte[]) javaObj)); break;
        default:
